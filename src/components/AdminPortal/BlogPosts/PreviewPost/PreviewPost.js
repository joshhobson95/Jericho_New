...
function PreviewPost() {
 ...
 const [previewData, setPreviewData] = useState([])
    ...
    useEffect(() => {
        axios.get(`https://jericho-new-test-rich-text.onrender.com/preview`)
        .then((response) => {
      ...
      const deletePreview = (previewtableid) => {
        axios
          .delete(`https://jericho-new-test-rich-text.onrender.com/deletepreview/${previewtableid}`, {
          })
          .then(() => {
            Swal.fire({
      ...
      const finalizePreview = (previewtableid) => {
        ...
        axios.post('https://jericho-new-test-rich-text.onrender.com/newblogpost', selectedPreview)
          .then((response) => {
...
<h1>{item.title2}</h1>
{item.img_3_url && <img src={item.img_3_url} className='blog_img' alt=''/>}
